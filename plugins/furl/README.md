...
- **MCP server** (`furl`, Claude Code + Codex) → the `furl_compress`, `furl_retrieve`,
  ...
  takes inline `content`, a local `file_path`, or an OpenAI/ChatGPT host-provided `file` attachment transferred out-of-band via `openai/fileParams`.
...
| `FURL_MCP_MAX_FILE_BYTES` | `41943040` (40 MiB) | Byte ceiling for both `furl_compress` file-ingest paths: local `file_path` disk ingest and OpenAI/ChatGPT host-provided `file` attachments (`openai/fileParams`). Separate from and larger than the 10 MiB inline-`content` cap. It is a real bound and never unbounded: a blank, non-numeric, or non-positive value falls back to the default. Set from measurement — ingest cost grows steeply with size (a ~34 MiB trace compresses in ~18–20 s, ~40 MiB in ~30 s, ~64 MiB in ~158 s), so 40 MiB clears the ~33 MiB target with headroom while a file at the ceiling still finishes inside the MCP tool-call budget; a larger file is refused fast and clearly. Raise it to ingest a bigger file, accepting the extra latency. |
