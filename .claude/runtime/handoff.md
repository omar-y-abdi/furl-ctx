...
**REMAINING backlog:** R7 (retention durable spill — wire Sqlite as eviction spill OPT-IN/default-OFF so default behavior unchanged = no regression; assess first whether the LIVE retrieval path already uses the Rust SqliteCcrStore prod-default, which may shrink this), D3 SmartCrusher MEASURE-not-delete (report which strategy ships; do NOT delete compaction — regresses floor). Then finish: full sanity gate+bench → rm codebase-CRITIQUE.md → rerun adversarial-critique.js UNCHANGED → delta report.
**Gate baseline now: suite 592/14, recovery 22, floor held on all 6 datasets, needle 100%. HEAD `af2ab84e`. PM-loop: agents edit-only, I run gate (+maturin first if Rust changed), commit on PASS, git add -u (never -A → critique stays untracked).**
