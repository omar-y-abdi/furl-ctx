...
**REMAINING backlog:** R6 (ContentRouter god-object split — HIGHEST regression risk: live hot path + thread-local _runtime_* coupling; critique itself says clean split is "partly naive" — extract only SAFE seams, leave genuinely-coupled core if no-regression can't be guaranteed), R7 (retention durable spill Sqlite default-off), D3 SmartCrusher MEASURE-not-delete. Then finish: sanity gate+bench → rm codebase-CRITIQUE.md → rerun adversarial-critique.js UNCHANGED → delta report.
**Gate baseline now: suite 592/14, recovery 22, floor held on all 6 datasets, needle 100%. HEAD `ff2b4ff2`.**
