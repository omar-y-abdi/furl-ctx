...
**SURFACED follow-ups (from agents, not yet done):** R-EXCISE-C = .github/workflows/ (17 files; many upstream-CI for amputated wrap/proxy/docker/publish/eval/network-diff features + ghcr.io/chopratejas refs) + dead `TelemetryBeacon` (telemetry/beacon.py — unwired, polls deleted /stats route) + deleted-proxy ROUTE refs in comments (/v1/retrieve, /stats, /v1/telemetry). archive/ still has chopratejas refs = OUT OF SCOPE (intentional).
**REMAINING backlog:** R-EXCISE-C, then critique R1 (clippy orchestration.rs:158), R3 (dead py twins adaptive_sizer.py+log/search _select_*), R4 (telemetry collapse, keep record_metrics), R5 (CCR marker grammar, HIGH), R6 (ContentRouter split, HIGH), R7 (retention durable spill), D3 SmartCrusher MEASURE. Then finish: sanity gate → rm critique → rerun adversarial-critique.js → delta report.
