...
- **BATCH-2 PROGRESS:** ✓ G1 COR-25/26/27 `93467935` (log-traceback / search-sep / diff-binary-regex; bench +0) · G2 COR-23/24 (parity: anchor char-count + constant-column score-field gate) RUNNING [cor2324]. Note: `scripts/build_rust_extension.sh` broken in the uv venv (no pip) → agents use `maturin develop --uv`; ci-precheck-python's Rust build step needs the same fix (infra follow-up).
