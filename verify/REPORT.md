...
> stamped 2026-07-03).** This report is a verification *record* of the tree
> as of engine commit `3a507010` / harness HEAD `d760d4a4`. It is kept
> verbatim for provenance; three things have drifted since:
>
> 1. **File:line anchors** (`measure.py:263/269/531`) refer to the audited
>    tree, not today's — use symbol names (`hash_compare_structured`,
>    `reset_compression_store`) to find the current sites.
> 2. **The `_present_in_text` scalar-substring fallback discussed in §4.1
>    and §5 has since been REMOVED** from `verify/measure.py` — the shipped
>    reconstruction is now *stricter* than the leniency this report
>    investigated (the fallback was shown non-load-bearing even when it
>    existed).
> 3. **`raw_results.json` is NOT committed** — it is regenerated by every
>    `python -m verify.run` sweep. "Byte-identical to raw_results.json"
>    below means identical to the file produced by the audited run, which
>    the auditor held locally; the claim is *re-runnable* (same seeds, same
>    data files), not verifiable from the repository alone.
...
- **Reproduced exactly:** my fresh re-run of search@90 medium gave
  ...
  audited run's `raw_results.json`** (not committed — regenerated per sweep;
  see the banner).
...
2. **search@90 all 6 seeds, medium** -> `[0.9375...0.9385]`, **byte-identical to
   the audited run's raw_results** (not committed; see banner). (Confirms
   determinism, no cherry-picking.)
