...
- `verify/heldout/` — second, disjoint, out-of-sample verification
  (express/chalk/npm-cli, seeds `2000+211·i`); harness since removed
  (see git history for `verify/heldout/run.py`/`strict_recheck.py`),
  audit `verify/heldout/REPORT.md`.
...
All tiers byte-exact. (harness since removed; see git history for
`verify/heldout/run.py`; cross-checked against `verify/run.py`.)
...
> but offloads to the CCR store (**95.9%**, retention 100%), and multiturn
> rose from its 70.8% dev headline to **84.5%** (structured-JSON
> error-protection fix). Both figures are the committed capture in
> `benchmarks/baseline_results.json`; current capture rendered at
> `benchmarks/BASELINE.md`; the tables below are dated records.
...
- **Lossless recovery is real and byte-exact.** Under the de-cheated strict
  ...
  violations. An independent `strict_recheck.py` spot-check (harness since
  removed; see git history) reconstructed purely from visible rows +
  `decode_csv_schema_rows` + CCR retrieve and confirmed
