...
first use with `uv run --no-project --with "furl-ctx[mcp]==1.4.0"`. Be explicit
about that posture (audit High-6):

- **Version-pinned, not hash-pinned.** The engine version is exact
  (`==1.4.0`), so you always get that release, but the download is not verified
...
- **Hardening path for high-assurance environments.** The plugin always resolves the
  engine through `uv run --no-project --with 'furl-ctx[mcp]==1.4.0'`. It does not consult
