...
# --------------------------------------------------------------------------- # Primary repro (mandated): crush a real array through SmartCrusher's
# row-drop path and confirm resolve_markers restores the exact original. --------------------------------------------------------------------------- #
...
# Pin byte-identical double-angle markers emitted by the Rust CCR producers for row offload, row indexing, and opaque recovery.
...
# --------------------------------------------------------------------------- # Pin: the bracket family (G/H) was already full-span; the fix must not change its behavior. Byte-identical
# literals from the Rust module's own diff_is_byte_identical / retrieve_more_is_byte_identical pins. --------------------------------------------------------------------------- #
...
# # Generic-bracket substitution span (the bracket-family sibling of T4) a GENERIC_BRACKET_PATTERN match must
# span exactly the marker's own "[...]" run, never reach back to an earlier innocent "[" on the same line.
...
# Pin the double-angle tail bound at its 64/65 boundary. The same bound prevents marker over-match and caps
# backtracking per failed start, giving a deterministic ReDoS guard without timing-sensitive assertions.
...
def test_forged_marker_cannot_swallow_more_than_bound_chars_of_filler() -> None:
    ...
    # Exactly at the bound (64 filler chars): still resolves -- the window is inclusive
    # up to the chosen bound, matching every real shape's tail (all far shorter than 64).
    ...
    # One char past the bound: the hash-shaped prefix must not match at all, so it must not reach through the
    # oversized filler to a distant later '>>' and swallow everything in between into a single substitution.
...
# Shape C's `kind` field is the only marker sub-shape whose text is not producer-fixed; guard the closing double-angle tail explicitly.
