...
# ── F8: effective retrieval headroom in the store block ─────────────────────


async def test_cap_accounting_reports_logical_vs_derived() -> None:
    store = resolve_ccr_namespace_store()
    store.store("parent", "<<ccr:a>>", explicit_hash="a" * 24)
    store.store("c1", "<<ccr:b>>", explicit_hash="b" * 24, derived_of="a" * 24)
    store.store("c2", "<<ccr:c>>", explicit_hash="c" * 24, derived_of="a" * 24)

    cap = _envelope(await FurlMCPServer()._handle_stats())["store"]["cap_accounting"]
    assert cap["logical_entries"] == 1
    assert cap["derived_entries_excluded"] == 2
    assert cap["retrieval_headroom"] == cap["max_entries"] - 1
    assert "warning" not in cap  # nowhere near the cap


def test_cap_accounting_block_warns_near_the_cap() -> None:
    # Static-method unit test so the threshold is checked without seeding 900 rows.
    below = FurlMCPServer._cap_accounting_block(899, 0, 1000)
    assert "warning" not in below
    at = FurlMCPServer._cap_accounting_block(900, 0, 1000)  # 90% == _CAP_WARN_RATIO
    assert "warning" in at and "headroom is low" in at["warning"]
    full = FurlMCPServer._cap_accounting_block(1000, 0, 1000)
    assert full["retrieval_headroom"] == 0 and "warning" in full


def test_cap_accounting_block_handles_zero_max() -> None:
    block = FurlMCPServer._cap_accounting_block(0, 0, 0)
    assert block["retrieval_headroom"] == 0
    assert block["fill_percent"] == 0.0
