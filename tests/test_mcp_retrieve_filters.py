...
@pytest.mark.parametrize(
    ("line_range", "key", "expected"),
    [
        ([2, 3], "filtered_content", "2:beta ERROR here\n3:gamma third line"),
        ([4, None], "filtered_content", "4:delta ERROR again\n5:epsilon last"),
        ...
        ([5, 2], "error", "line_range end (2) must be >= start (5)"),
        ([0, 3], "error", "line_range start must be >= 1, got 0"),
    ],
    ids=["window", "open-end", "inverted", "below-one"],
)
async def test_line_range_projection_and_bounds(server, line_range, key, expected) -> None:
    _seed(server, "a" * 12, _TEXT)
    env = _envelope(await server._handle_retrieve({"hash": "a" * 12, "line_range": line_range}))
