...
- `ccr/backends/mod.rs:85` — `from_config` — InMemory/Sqlite (Redis config → loud `UnsupportedBackend`), no silent fallback.
...
- **apply() kwargs allowlist (typo guard):** `ContentRouter.apply` (`content_router.py:1420`) rejects any kwarg not in the module-level `_APPLY_ALLOWED_KWARGS` frozenset (`content_router.py:510`), so a misspelled per-request option fails loud instead of being silently ignored. Per-request options (`target_ratio`/`force_kompress`/`kompress_model`) are read off kwargs into the frozen `RouterRuntime` value via `RouterRuntime.from_kwargs` (`content_router.py:114`) and threaded by argument down compress/dispatcher/`_try_ml_compressor`/`_record_to_toin` — no thread-local, no main→worker replay (workers receive the same immutable value by argument).
...
Notes: `cargo test` cannot run the `headroom-py` cdylib (`test=false` in Cargo.toml) — Python-side tests only. The core is ML-free with no feature flags (`default = []`); the ML backends (magika/embeddings, ONNX `ort`) and the Redis backend were excised — relevance is BM25-only and a Redis `CcrBackendConfig` returns a loud `UnsupportedBackend` error. Default model gpt-4o (real tiktoken); benchmarks use RoutingPolicy.MinTokens with CompressConfig defaults.