...
- `headroom/transforms/router_dispatch.py:43/65` — `StrategyDispatcher` (`apply`) — per-strategy compressor dispatch + the SMART_CRUSHER→KOMPRESS→LOG→passthrough no-savings fallback chain. `ContentRouter._apply_strategy_to_content` (`content_router.py:1099`) is now a thin delegator that resolves the compressor-getters fresh and binds `_try_kompress`/`_record_to_toin` (with per-call `runtime`) into closures passed in per call.
- `headroom/transforms/router_ccr_mirror.py:47/59/137` — `CcrMirror` (`ensure_ccr_backed`/`extract_ccr_hashes`) — result-cache HIT re-mirror of `<<ccr:HASH>>` pointers back into the Python store + hash extraction. `ContentRouter._ensure_ccr_backed`/`_extract_ccr_hashes` (`content_router.py:1268/1286`) are thin delegators.

**public API**
- `headroom/compress.py:197` — `compress` — one-liner entry; inflation guard reverts if tokens grow (`compress.py:318`).
- `headroom/compress.py:77/138` — `CompressConfig`/`CompressResult` — config + metrics.
...
- Change content routing / per-type dispatch → `content_router.py:778` (ContentRouter.compress orchestrator), `content_router.py:1099` (`_apply_strategy_to_content`), `content_router.py:255/273` (Rust detect + regex fallback), `content_detector.rs:221` (detect_content_type).
...
- **apply() kwargs allowlist (typo guard):** `ContentRouter.apply` (`content_router.py:1403`) rejects any kwarg not in the module-level `_APPLY_ALLOWED_KWARGS` frozenset (`content_router.py:554`), so a misspelled per-request option fails loud instead of being silently ignored. Per-request options (`target_ratio`/`force_kompress`/`kompress_model`) are read off kwargs into the frozen `RouterRuntime` value via `RouterRuntime.from_kwargs` (`content_router.py:118`) and threaded by argument down compress/dispatcher/`_try_kompress`/`_record_to_toin` — no thread-local, no main→worker replay (workers receive the same immutable value by argument).
