...
| `FURL_CCR_SQLITE_MAX_ROWS` | `10000` | Physical row cap for the SQLite file (oldest-created evicted first) — a retention backstop an order of magnitude above the 1000-entry logical cap at the defaults. One compression is one row, so at the default caps the logical cap binds first; this only guards the shared file's on-disk size. Both caps are configurable — setting this below the store's `max_entries` inverts the ordering (the store logs a warning when it detects that). |
...
| `FURL_MCP_MAX_FILE_BYTES` | `41943040` (40 MiB) | Byte ceiling for both `furl_compress` file-ingest paths: local `file_path` (jailed to `FURL_WORKSPACE_DIR` through the same fd-pinned walk as `furl_read`) and host-provided `file` attachments such as ChatGPT/OpenAI `openai/fileParams` uploads, downloaded out-of-band. Separate from and larger than the 10 MiB inline-`content` cap because ingesting a large artifact is the point (a 33 MB trace that overflows context if pasted inline). A real DoS bound, never unbounded: blank/non-numeric/non-positive falls back to the default. Set from measurement — compression cost is super-linear in size (~34 MiB ingests in ~18–20 s, ~40 MiB ~30 s, ~64 MiB ~158 s), so 40 MiB clears the ~33 MiB target with headroom while a file at the ceiling still returns inside the MCP tool-call budget, and anything larger is refused fast rather than left to hang. Positive integer; raise it to ingest a bigger file at the cost of latency. |
