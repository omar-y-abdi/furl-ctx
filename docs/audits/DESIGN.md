...
3. **Result**: 90 rows identical-except-timestamp collapse to one stable hash → dedup + cluster + fill-diversity work as intended → log/structured compression rises from ~30% toward 82-95%. The kept representative renders the varying-identity columns that drove the collapse as the reserved `<varies>` sentinel and keeps its constant content; multiplicity recorded as `_dup_count`, with the concrete varying values always CCR-recoverable from the full-original store.
