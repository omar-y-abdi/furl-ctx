...
| 2026-07-25 | correctness, diff compressor chmod-only silent loss | crates/furl-core/src/transforms/diff_compressor.rs | resolves the chmod half of the 2026-07-24 open candidate below: `parse_diff` recognized `new file mode`/`deleted file mode`/rename/binary marker lines but had no branch at all for `old mode <NNNNNN>`/`new mode <NNNNNN>` (git's format for a permission-only change, e.g. `chmod +x`, or a type change like regular-file <-> symlink via 100644 <-> 120000) — these lines matched no parser branch, so a chmod-only file section (no `---`/`+++`, no hunks, matching real `git diff` output verified against an actual repo) reassembled to nothing past its `diff --git` header, and a mode change combined with real content edits silently lost just the mode pair. Below the 0.8 `min_compression_ratio_for_ccr` gate no CCR marker is emitted either, so the loss is not merely CCR-deferred, it is genuinely unrecoverable: reproduced live with a diff whose only shrinkage was the 2 dropped mode lines out of 54 (ratio 0.96, comfortably above the gate) — `cache_key` came back `None` with the permission bit gone from both the visible output and the store. Fix: two new `DiffFile` fields (`original_old_mode_line`/`original_new_mode_line`) capture the lines verbatim in `parse_diff`, and `format_output` re-emits them in git's canonical order (after any new-file/deleted-file mode line, mutually exclusive with it; before `---`/`+++`/hunks when content also changed). 2 new tests, both red-proofed by temporarily gating the two emission lines behind `if false` and confirming both fail with the exact deletion signature, then restored: one pins the true-unrecoverable mode-only case (asserts `cache_key.is_none()` so the test can't pass for the wrong reason), the sibling pins mode+content combined in one file section. Full gate green: `cargo test --workspace` 848→850 passed 0 failed, `cargo clippy -D warnings`/`cargo fmt --check` clean, pytest 2737 passed/19 skipped (pre-existing, unrelated — `_core.abi3.so` rebuilt via `scripts/build_rust_extension.sh` before baselining), `verify.run` unchanged at `degradations=6 hash_failures=0 silent_loss=0 cache_prefix_violations=0`, `compare_baseline` 0 regressions/0 improvements (no benchmark corpus contains a chmod-only or mode+content diff, which is why dedicated Rust-level repros were needed). Deliberately not done: the `index …` line drop (no semantic value, just a blob SHA) and Q7's documented-deliberate mode/binary normalization, both explicitly scoped out of this candidate | #169 |

| 2026-07-24 | security, built-in private-key redaction leaked the key material | furl_ctx/redaction.py, tests/test_redaction_private_key_blocks.py, tests/test_redaction_env.py, SECURITY.md, LIBRARY.md | resolves the "strongest runner-up" open candidate below. The ON-by-default `private-key` built-in matched ONLY the `-----BEGIN … PRIVATE KEY-----` armor line: the armor is the ANCHOR, not the credential, so every base64 byte of the key survived into the model-visible compressed content AND into the CCR store, sitting directly under a `[REDACTED:private-key]` marker asserting the opposite — false assurance, worse than no redaction. Reproduced deterministically (twice, identical) at three levels before any fix: `build_default_redactor({})` left 3/3 body lines for all 5 label variants; `compress()` on a Bash-shaped payload put the material in `result.messages[0]["content"]` (the model reads the key); and the mcp_server pre-store sequence stored it so `retrieve(hash)` returned 24/24 key lines. Reach is every store path — PostToolUse hook, Bash pipe, `furl_compress`/`furl_read`, CLI, library `compress()` — plus `furl list`/`furl search` previews, whose contract is "a listing must not print a secret back out". Oracles: SECURITY.md and LIBRARY.md ("private keys … scrubbed from the stored original **before** it is compressed or written to disk"), `mcp_server._furl_compress`'s own comment, and the sibling `compression_store._PEM_PRIVATE_KEY_RE`, which has always taken the whole block on the narrower retrieval-log surface. Fix: a two-branch pattern — full block (`BEGIN` armor → interior → `END` armor) with a fallback that takes the armor plus any hanging base64 run so a tool-truncated block cannot leak either. The interior is `(?:[^-]|-(?!----)){0,100000}`, "any char that does not open a 5-hyphen armor run", chosen over a plain `[\s\S]*?` for BOTH correctness (an unterminated key cannot reach forward to a later key's `END` and destroy the logs between) and cost: measured 12.14 s vs 5 ms on 310 KB of packed bare armors, which matters because the hook's 30 s timeout is fail-OPEN (a hang degrades redaction to raw passthrough). `[A-Z0-9 ]*` before `PRIVATE KEY` also picks up `ENCRYPTED` PKCS#8, and an optional ` BLOCK` suffix picks up PGP, whose real armor is `BEGIN PGP PRIVATE KEY BLOCK` — the old alternation listed a `PGP ` label but spelled the tail `PRIVATE KEY-----`, so it could never match a PGP export at all. Deliberate, pinned narrowings: mismatched BEGIN/END labels over-redact the span rather than leak, and the truncated branch's 16-char base64 floor leaves prose under a documentation armor byte-exact. 27 new tests in a dedicated file (labels, PGP, JSON-escaped `\n` bodies, CRLF, RFC-1421 `Proc-Type`/`DEK-Info` headers, truncation, marker-length independence, over-redaction boundaries, public material, a hyphen-rule guard, an e2e that asserts on the model-visible content, every stored entry, every retrieval, and the raw sqlite bytes on disk), plus the existing `test_builtins_redacts_private_key_block` strengthened — it asserted only the armor's absence, which is exactly why a header-only pattern passed it for months. Red-proof: reverting the one production line reddens 20 of the 28 (the 8 that stay green are the parity, over-redaction and cost guards, correct by construction); the ReDoS guard was separately red-proofed against the naive `[\s\S]*?` interior at 12.14 s vs its 5.0 s ceiling. Full gate green: pytest 2714→2741 passed/19 skipped at first review, cargo 848+17+5+3+5, clippy/fmt/ruff/mypy clean, `maturin sdist`+`build --release`+`twine check` PASSED, `verify.run` unchanged at `degradations=6 hash_failures=0 silent_loss=0 cache_prefix_violations=0`, compare_baseline 0 regressions 0 improvements (no benchmark corpus carries a PEM block). Phase 0 note: `_core.abi3.so` was again stale (Jul 18 build vs Rust merges through #164) and was rebuilt via `maturin develop --release --extras dev,mcp` before baselining — the fourth session running to hit this. Post-review follow-up (F-1, found by an independent adversarial review on the PR and confirmed by re-running the reviewer's own repro): the truncated branch required a leading NEWLINE before EVERY base64 run including the first, so a truncated body abutting the armor with no separator, or with only a space or a tab, still leaked its material in cleartext under the marker — a real shape once newlines have been stripped, which is how a single-line env-var key arrives (complete blocks were always clean in every shape, including all-on-one-line, since the block interior never required newlines; it was an incomplete fix, not a regression, since main leaked all three shapes identically). Fixed by making only the FIRST run's separator optional, so multi-line precision is untouched; the disclosed cost is that a 16+ character unbroken alphanumeric word abutting the armor is now eaten, at most that one run, pinned by `test_abutting_run_at_or_past_the_floor_is_the_disclosed_over_redaction`. 13 more tests (7 separator variants, the floor boundary re-parametrized over both abutting and newline-separated runs, prose-abutting precision, and the two previously-unpinned magic numbers — `{0,100000}` falls back to the truncated branch rather than going unredacted, `{0,2048}` leaves exactly the 2049th run), red-proofed against the pre-F-1 pattern: exactly 6 fail, the newline/CRLF/escaped-newline variants correctly stay green because they were never broken. Cost profile re-measured and unchanged (6.51 ms vs 5.41 ms on the 10k packed-armor input). Final gate: pytest 2714→2754 passed/19 skipped, cargo 848+17+5+3+5, verify.run and compare_baseline unchanged | #166 |
| 2026-08-08 | build / CI, Ruff Markdown formatter compatibility | pyproject.toml, uv.lock, .github/workflows/{ci,autofix}.yml, LIBRARY.md | upgraded the Ruff floor and CI pins to 0.16.2, then formatted the Markdown examples that made the required lint gate fail on clean main | pending |
...
- 2026-07-24 (span-safety session hunt, EXECUTED proof by a review agent —
  ...
  The private-key half is RESOLVED as of 2026-07-24 (see the Completed row):
  the pattern now takes the whole block, with a truncated-block fallback, and
  the blind test asserts the material's absence. The two lower-severity
  siblings above are still OPEN and were deliberately not folded in — they
  are a different root cause (over- and under-MATCHING a token, not matching
  an anchor instead of the secret) and the `openai-key` one needs its own
  false-positive corpus before any narrowing, exactly as #150 needed for
  `field_role.rs`. A third sibling surfaced while fixing this one and is also
  open: every built-in pattern was audited for the anchor-vs-secret defect and
  only `private-key` had it, but that audit was by reading, not by a corpus
  sweep of real credential shapes per provider.
...
- 2026-07-24 (span-safety session hunt, diff/search compressor sweep):
  ...
  Resolved 2026-07-25 for the chmod-only half (see the Completed row):
  `old mode`/`new mode` lines are now captured and re-emitted verbatim,
  both mode-only (no hunks at all) and combined with a real content edit
  in the same file section. The `index …` line drop and Q7's mode/binary
  normalization are deliberately untouched — `index` carries only a blob
  SHA with no semantic value to a compressing LLM tool, and Q7 is a
  separate owner decision this session didn't have standing to revisit.
