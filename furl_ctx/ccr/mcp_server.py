...
# F8: warn in furl_stats once the shared store's LOGICAL fill reaches this
# fraction of the cap, so a session sees eviction pressure BEFORE the oldest
# retrievable markers start loud-missing within their TTL.
_CAP_WARN_RATIO = 0.9
...
class FurlMCPServer:
    ...
    def _store_derived_stats(self) -> dict[str, Any]:
        ...
        # F8: the cap is enforced on LOGICAL entries (primary compressions), not
        # raw rows — a columnar row-drop's per-row chunks are excluded — so the
        # real retrieval headroom is logical_entry_count vs max_entries.
        logical_entries = store_stats.get("logical_entry_count", store_stats.get("entry_count", 0))
        derived_entries = store_stats.get("derived_entry_count", 0)
        ...
        block: dict[str, Any] = {
            ...
            # F8 effective retrieval headroom: the cap counts LOGICAL entries
            # (primary compressions), not the per-row chunks a columnar row-drop
            # offloads, so one structured output is one slot rather than 1 + N.
            # ``retrieval_headroom`` is what a reader should watch — the number of
            # further logical compressions that fit before FIFO eviction starts
            # dropping the OLDEST retrievable content within its 24h TTL.
            "cap_accounting": self._cap_accounting_block(
                logical_entries, derived_entries, max_entries
            ),
    ...
    @staticmethod
    def _cap_accounting_block(
        logical_entries: int, derived_entries: int, max_entries: int
    ) -> dict[str, Any]:
        """F8 effective-headroom readout for the shared store's FIFO cap.

        The store caps on LOGICAL entries (one per compression); the per-row
        chunks a columnar row-drop offloads are excluded, so a handful of
        structured outputs no longer burn several slots each. ``retrieval_headroom``
        is how many more logical compressions fit before FIFO eviction begins
        dropping the OLDEST retrievable content — the real answer to "how long
        will my markers resolve", which the raw 24h TTL overstates. A ``warning``
        appears once the fill crosses ``_CAP_WARN_RATIO``.
        """
        headroom = max(0, max_entries - logical_entries) if max_entries else 0
        fill_percent = round(logical_entries / max_entries * 100, 1) if max_entries > 0 else 0.0
        block: dict[str, Any] = {
            "scope": (
                "logical entries (one per compression) vs the FIFO cap; the "
                "derived per-row chunks a columnar row-drop offloads are counted "
                "separately in derived_entries_excluded, NOT against the cap"
            ),
            "logical_entries": logical_entries,
            "derived_entries_excluded": derived_entries,
            "max_entries": max_entries,
            "retrieval_headroom": headroom,
            "fill_percent": fill_percent,
        }
        if max_entries > 0 and logical_entries >= _CAP_WARN_RATIO * max_entries:
            block["warning"] = (
                f"retrieval headroom is low: {logical_entries} of {max_entries} logical "
                f"slots used, {fill_percent}% full. Past the cap the store evicts the "
                f"OLDEST entries first, so the oldest <<ccr:...>> markers can loud-miss "
                f"well within their 24h TTL. Retrieve or purge what you no longer need."
            )
        return block
